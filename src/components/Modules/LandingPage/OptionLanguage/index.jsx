...
const OptionLanguage = () => {
    const [language, setLanguage] = useAtom(languangeAtom)
    ...
    return (
        <div className='relative bg-[#1A1510] text-white rounded border border-gray-400 w-[200px] pl-8'>
            ...
            <select onChange={handleSelect} value={language} className='w-full bg-transparent y-1'>
                <EachUtils of={LIST_LANGUAGE}
                    render={(item, index) => <option className='bg-black' key={index} value={item.value}>{item.name}</option>}
                />
            </select>
        </div>
    )
